...
const PrintPayslip = () => {
  ...
  useEffect(()=>{
    setPayslip(dummyPayslipData.find((slip)=>slip._id === slip.id))
    setTimeout(()=>{
      setLoading(false)
    },1000)
  },[id])
         ...
         <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr className="border-t-2 border-slate-200 bg-slate-50">
                <td className="py-4 px-4 text-slate-900">Deductions</td>
                <td className="text-right py-3 px-4 text-slate-900 font-bold text-lg">${payslip.netSalary?.toLocaleString()}</td>
              </tr>
            </tbody>
         </table>
        </div>
